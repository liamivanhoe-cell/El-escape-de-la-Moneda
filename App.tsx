...
import { Lock, Trophy, Users, Play, Gift, ArrowRight, Skull, Save, ShoppingCart, Coins, BriefcaseMedical, CircleOff, Info, Hammer, Sword, Music, Swords, Star, Globe, Volume2, VolumeX, Flame, Droplets, Clock, Zap, BookOpen, KeyRound, Box, User, Move, Crosshair, Shield } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
     ...
     if (screen === Screen.INTRO || screen === Screen.MENU || screen === Screen.LEVEL_SELECT || screen === Screen.COLLECTION || screen === Screen.TUTORIAL) {
         audioManager.playTheme('MENU');
  ...
  if (screen === Screen.TUTORIAL) {
      return (
          <div className="min-h-screen bg-slate-900 p-6 flex flex-col items-center overflow-auto">
              <div className="flex justify-between w-full max-w-5xl mb-6 sticky top-0 bg-slate-900/90 z-10 py-4 backdrop-blur">
                  <button onClick={() => setScreen(Screen.MENU)} className="text-slate-400 hover:text-white font-bold text-lg">← {TRANSLATIONS[language].BACK}</button>
                  <h2 className="text-3xl font-bold text-cyan-400 pixel-font flex items-center gap-3"><BookOpen size={32} /> {TRANSLATIONS[language].TUTORIAL}</h2>
                  <div className="w-20"></div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-5xl pb-10">
                  
                  {/* Card 1: Controls */}
                  <div className="bg-slate-800 p-6 rounded-2xl border-2 border-slate-600 shadow-xl">
                      <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><Move className="text-yellow-400"/> Movimiento y Acción</h3>
                      <div className="space-y-4 text-slate-300">
                          <p><strong className="text-white">Flechas / WASD:</strong> Mover al personaje.</p>
                          <p><strong className="text-white">ESPACIO:</strong> Mantén presionado para abrir Cofres y obtener el Martillo.</p>
                          <p><strong className="text-white">CTRL:</strong> Mantén para <strong>Agacharte</strong> (Esquiva rayos de jefes).</p>
                          <p><strong className="text-white">SHIFT:</strong> Disparar con la Pistola (Solo en Niveles de Jefe y Nivel 17).</p>
                      </div>
                  </div>

                  {/* Card 2: Goal */}
                  <div className="bg-slate-800 p-6 rounded-2xl border-2 border-green-600 shadow-xl">
                      <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><KeyRound className="text-green-400"/> Objetivos</h3>
                      <div className="space-y-4 text-slate-300">
                          <p>Empiezas en la <strong className="text-green-400">S</strong>. Debes llegar a la <strong className="text-yellow-400">E (Salida)</strong>.</p>
                          <p>Algunas puertas están bloqueadas. Busca pistas de código <span className="bg-cyan-900 text-cyan-200 px-1 rounded text-xs">📄</span> para abrirlas.</p>
                          <p>Rompe paredes agrietadas con el <strong>Martillo</strong>.</p>
                          <p>En el Nivel 6, debes destruir todos los bloques para salir.</p>
                      </div>
                  </div>

                  {/* Card 3: Enemies & Combat */}
                  <div className="bg-slate-800 p-6 rounded-2xl border-2 border-red-600 shadow-xl">
                      <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><Skull className="text-red-400"/> Enemigos y Combate</h3>
                      <div className="space-y-4 text-slate-300">
                          <p><span className="text-2xl">🐀</span> <strong>Ratas:</strong> Evítalas o usa Veneno/Queso.</p>
                          <p><span className="text-2xl">🦹</span> <strong>Humanos:</strong> En la ciudad. Evítalos o usa Cuchillo/Flauta.</p>
                          <p><span className="text-2xl">🤖</span> <strong>Animatrónicos:</strong> En la Pizzería (Nivel 21). Usa la Cubeta de Agua.</p>
                          <p><strong className="text-red-400">BOSSES:</strong> Tienen mecánicas únicas. Usa la munición correcta.</p>
                      </div>
                  </div>

                  {/* Card 4: Ammo & Items */}
                  <div className="bg-slate-800 p-6 rounded-2xl border-2 border-purple-500 shadow-xl">
                      <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><Crosshair className="text-purple-400"/> Munición e Items</h3>
                      <div className="space-y-4 text-slate-300">
                          <p>Usa las teclas <strong>1, 2, 3</strong> para cambiar de bala:</p>
                          <ul className="list-disc list-inside pl-4 text-sm">
                              <li><strong>Normal:</strong> Daño base.</li>
                              <li><strong>Fuego:</strong> Doble daño (Destruye Cajas Rojas).</li>
                              <li><strong>Queso:</strong> Aturde y daño alto (Destruye Cajas Amarillas).</li>
                          </ul>
                          <p className="mt-2"><strong className="text-orange-400">Estrella Mágica:</strong> Te hace invencible temporalmente. ¡Muy rara!</p>
                      </div>
                  </div>

                  {/* Card 5: Shop */}
                  <div className="bg-slate-800 p-6 rounded-2xl border-2 border-yellow-500 shadow-xl md:col-span-2">
                       <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><ShoppingCart className="text-yellow-400"/> La Tienda</h3>
                       <p className="text-slate-300">
                           Gana <strong className="text-yellow-400">Oro</strong> completando niveles. Úsalo para comprar ventajas antes de entrar a niveles difíciles.
                           ¡En <strong>Hardcore</strong> todo es más caro y difícil!
                       </p>
                  </div>
              </div>
          </div>
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center p-6 text-center">
            ...
            <div className="space-y-4 w-full max-w-xs">
              ...
              <button 
                ...
                className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3 rounded-xl shadow-[0_4px_0_#0e7490] active:shadow-none active:translate-y-1 transition-all flex items-center justify-center gap-2"
              >
                <BookOpen size={20} /> {TRANSLATIONS[language].TUTORIAL}
              </button>
            </div>
        </div>
