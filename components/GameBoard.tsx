...
export const GameBoard: React.FC<GameBoardProps> = ({ levelData, players, inventory, gameMode, language, isHardcore, onWin, onExit, onGameOver, onUseItem, onGetItem }) => {
  ...
  const endBossMode = (victory: boolean, isDream: boolean = false) => {
      ...
      // If Victory OR Dream Death -> Remove the portal
      if (victory || isDream) {
           setLocalTiles(prev => prev.map(t => t.type === 'secret_door' ? { ...t, type: 'floor' } : t));
           setActivePlayers(curr => curr.map(p => {
                const saved = savedPlayers.find(sp => sp.id === p.id);
                // Restore original position
                return { 
                    ...p, 
                    x: saved ? saved.x : levelData.startPos.x, 
                    y: saved ? saved.y : levelData.startPos.y, 
                    lives: saved ? saved.lives : p.lives, // Restore lives if it was a dream? Or keep damage? Let's restore to be safe as it was a "dream"
                    isDucking: false 
                };
           }));
           ...
           // Normal failure (shouldn't happen with dream logic taking over death, but fallback)
           setActivePlayers(curr => curr.map(p => {
                const saved = savedPlayers.find(sp => sp.id === p.id);
  ...
  const handlePlayerDamage = useCallback((playerId: number, reason: string, damage: number = 1) => {
    ...
    setActivePlayers(prev => {
        ...
        if (player.lives <= 0) {
            // SPECIAL LOGIC FOR SECRET BOSS ROOM DEATH
            if (isBossMode) {
                 // Trigger Dream Death
                 setTimeout(() => {
                    showMessage("Oh, parece que ha sido un sueño...", "text-purple-300");
                    endBossMode(false, true); // fail, but dream
                 }, 0);
                 return prev; // Do not set to dead, endBossMode will reset
            }
